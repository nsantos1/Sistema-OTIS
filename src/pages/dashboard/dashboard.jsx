...
/**
 * Retorna o nome da classe CSS para o status da atividade.
 * @param {string} status - O status da atividade (ex: "Resolvido", "Fora do prazo").
 * @returns {string} - A string formatada para uso na classe CSS (ex: "resolvido", "fora-do-prazo").
 */
...
function Dashboard() {
    ...
    // Efeito para sincronizar o estado 'selectedMonth' com a URL (query param)
    useEffect(() => {
    ...
    // Efeito para definir o mês inicial como o mais recente, se não houver na URL
    useEffect(() => {
    ...
    const filteredMetrics = useMemo(() => {
        ...
        // Se não houver produtos, evita divisão por zero
            ...
            <div className="dashboard-kpi-content">
                <header className="dashboard-header">
                    ...
                    <div className="dashboard-filters">
                        ...
                        {/* Filtro de SETOR - Aprimorado com .filter-group e label */}
                        <div className="filter-group">
                            <label htmlFor="setor">SETOR</label>
                            <select id="setor" name="setor" className="filter-select">
                                <option value="">Todos</option>
                                {setorOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                            </select>
                        </div>

                        {/* Filtro de UNIDADE - Aprimorado com .filter-group e label */}
                        <div className="filter-group">
                            <label htmlFor="unidade">UNIDADE</label>
                            <select id="unidade" name="unidade" className="filter-select">
                    ...
                    <div className="chart-container">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={prazoPorAreaData} layout="vertical" margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                                ...
                                <Bar dataKey="dias" barSize={15} radius={[0, 10, 10, 0]}>
                        ...
                        <ResponsiveContainer width="100%" height={250}>
                            <PieChart>
                                <Pie
                                    ...
                                    cx="50%"
                                    cy="50%"
                                    labelLine={false} // Label line removida para evitar poluição visual
                    ...
                    <div className="chart-container">
                        ...
                        <ResponsiveContainer width="100%" height={250}>
                            <RadarChart cx="50%" cy="50%" outerRadius={80} data={desempenhoPorAreaData}>
                                <PolarGrid stroke="#cdcdcd" /> {/* Adicionado stroke para PolarGrid */}
                    ...
                    <div className="chart-container full-width">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={evolucaoPrazoData} margin={{ top: 5, right: 30, left: -10, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /> {/* Adicionado stroke para CartesianGrid */}
                        ...
                        <tbody>
                            {atividadesRecentesData.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        {/* USO DA FUNÇÃO AUXILIAR PARA A CLASSE DO BADGE */}
                                        <span className={`status-badge status-${getStatusBadgeClass(item.status)}`}>
                                            {item.status}
                                        </span>
                                    </td>
                                    <td>{item.data}</td>
