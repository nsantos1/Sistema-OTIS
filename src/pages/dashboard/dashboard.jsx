...
// =================================================================
// DADOS DE PRODUÇÃO (MOCK DATA para Custo e Qualidade)
// =================================================================
...
// Helper: extrai lista de meses únicos em formato YYYY-MM e label legível
const getMonthsFromData = (data) => {
  const s = Array.from(new Set(data.map(d => {
  ...
  return s.map(ym => {
    ...
    const mesLabel = date.toLocaleString('pt-BR', { year: 'numeric', month: 'short' }); // ex: "ago. de 2025" ou "ago de 2025" dependendo do ambiente
    return { mes: ym, mesLabel };
  });
};

// util: dado "YYYY-MM" retorna [startDate, endDateExclusive]
const monthRangeToDates = (ym) => {
  if (!ym) return [null, null];
  const [year, month] = ym.split('-').map(Number);
  const start = new Date(year, month - 1, 1, 0, 0, 0, 0);
  const end = new Date(year, month, 1, 0, 0, 0, 0); // exclusivo
  return [start, end];
};
...
function Dashboard() {
  ...
  // months list derivada do productionData
  const monthsList = useMemo(() => getMonthsFromData(productionData), []);

  // initial month: da query param 'month' ou mês mais recente do data
  const initialMonthParam = searchParams.get('month') || (monthsList[0] && monthsList[0].mes) || '';
  const [selectedMonth, setSelectedMonth] = useState(initialMonthParam);

  // sincroniza selectedMonth com query param
  useEffect(() => {
  ...
  // quando monthsList disponível e não existe selectedMonth definimos o último (mais recente)
  useEffect(() => {
  ...
  // 2. Lógica de Filtragem e Agregação (BI - useMemo)
  const filteredMetrics = useMemo(() => {
    ...
    // média ponderada de retrabalho (já em %)
